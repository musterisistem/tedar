        ...
        <div className="space-y-8">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-xl font-normal text-slate-900 tracking-tight uppercase">Sepette İndirim Yönetimi</h1>
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <h2 className="text-lg font-normal text-slate-800">İndirim Oranı Ayarları</h2>
                </div>
                <div className="max-w-xs">
                    <label className="block text-sm font-normal text-slate-700 mb-2">Çoklu Alım İndirim Oranı (%)</label>
                    <div className="relative">
                        <input
                            type="number"
                            value={localRate}
                            onChange={(e) => setLocalRate(Number(e.target.value))}
                            className="w-full pl-4 pr-12 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-blue-500/10 outline-none transition-all font-normal text-slate-800"
                        />
                        <span className="absolute right-4 top-1/2 -translate-y-1/2 font-normal text-slate-400">%</span>
                    </div>
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col h-[600px]">
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-3">
                            ...
                            <h2 className="text-lg font-normal text-slate-800">Ürün Seçimi</h2>
                        </div>
                        <span className="text-xs font-normal text-slate-400">{filteredProducts.length} Ürün Bulundu</span>
                    </div>
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <h4 className="text-sm font-normal text-slate-800 truncate">{product.name}</h4>
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col h-[600px]">
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-3">
                            ...
                            <h2 className="text-lg font-normal text-slate-800">Seçili Ürünler (Maks 5 Görünür)</h2>
                        </div>
                        <span className="px-2 py-1 bg-green-50 text-green-600 text-xs font-normal rounded-lg">{localProductIds.length} Ürün</span>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
                        {selectedProducts.length > 0 ? (
                            selectedProducts.map(product => (
                                <div key={product.id} className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100 relative group animate-in slide-in-from-right-4 duration-300">
                                    ...
                                    <div className="flex-1">
                                        <h4 className="text-sm font-normal text-slate-800">{product.name}</h4>
                                        <div className="flex items-center gap-3 mt-1">
                                            <span className="text-xs font-normal text-blue-600 bg-blue-50 px-2 py-0.5 rounded">Sepette İndirimli</span>
                                            <span className="text-xs font-normal text-slate-400">{product.price.current.toLocaleString('tr-TR')} TL</span>
                                        </div>
                                    </div>
                                    <button
                                        onClick={() => toggleProduct(product.id)}
                            ...
                            <div className="h-full flex flex-col items-center justify-center text-center p-8 opacity-40">
                                ...
                                <p className="text-sm font-normal">Henüz ürün seçilmedi</p>
