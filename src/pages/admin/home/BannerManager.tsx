        ...
        <div className="space-y-8">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-xl font-normal text-slate-900 tracking-tight uppercase">Banner ve Kampanyalar</h1>
            ...
            <div className="flex bg-white p-1 rounded-2xl border border-slate-100 max-w-lg mx-auto">
                <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-normal transition-all ${activeTab === 'banners' ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}
                >
                    <Layout className="w-4 h-4" /> REKLAM BANNERLARI
                </button>
                <button
                    onClick={() => setActiveTab('campaigns')}
                    className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-normal transition-all ${activeTab === 'campaigns' ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}
                >
            ...
            {activeTab === 'banners' ? (
                    ...
                    <div className={`p-6 rounded-xl border ${editingBannerId ? 'bg-blue-50 border-blue-200' : 'bg-white border-slate-200'}`}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="md:col-span-2">
                                ...
                                <div className="flex items-center gap-4">
                                    <input ref={fileInputRef} type="file" accept="image/*" onChange={(e) => handleImageUpload(e, 'banner')} className="block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-normal file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" />
                        ...
                        {banners.map((b) => (
                            <div key={b.id} className="flex items-center gap-4 p-4 bg-white border rounded-lg shadow-sm">
                                ...
                                <div className="flex-1">
                                    <h3 className="font-normal">{b.title || 'İsimsiz'}</h3>
                ...
                <div className="space-y-6 animate-fadeIn">
                    <div className={`p-6 rounded-xl border ${editingCampaignId ? 'bg-orange-50 border-orange-200' : 'bg-white border-slate-200'}`}>
                        <h2 className="text-lg font-normal mb-4 flex items-center gap-2 text-orange-800">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="md:col-span-2">
                                ...
                                <div className="flex items-center gap-4">
                                    <input ref={fileInputRef} type="file" accept="image/*" onChange={(e) => handleImageUpload(e, 'campaign')} className="block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-normal file:bg-orange-50 file:text-orange-700 hover:file:bg-orange-100" />
                    ...
                    <div className="space-y-3">
                        {campaigns.map((c) => (
                            <div key={c.id} className="flex items-center gap-4 p-4 bg-white border rounded-lg shadow-sm">
                                ...
                                <div className="flex-1">
                                    <h3 className="font-normal">{c.title}</h3>
