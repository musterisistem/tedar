    ...
    if (!order) {
        return (
            <div className="p-8 text-center">
                ...
                <button onClick={() => navigate('/admin/orders')} className="text-blue-600 font-normal hover:underline">Listeye Dön</button>
            </div>
        );
    }
        ...
        <>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <div className="flex items-center gap-3">
                                <h1 className="text-2xl font-normal text-slate-800">Sipariş #{order.orderNo}</h1>
                                {/* Status Configurator */}
                                <div className="flex items-center gap-2">
                                    <select
                                        ...
                                        className={`text-xs font-normal px-2 py-1 rounded border outline-none cursor-pointer ${statusInfo.color}`}
                                    >
                                    ...
                                    {isDirty && (
                                        <button
                                            ...
                                            className="text-xs bg-blue-600 text-white px-2 py-1 rounded font-normal hover:bg-blue-700 transition-colors animate-fadeIn"
                                        >
                                            Kaydet
                                        </button>
                                    )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                                ...
                                {orderItems.map((item: any) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex-1">
                                            <h3 className="font-normal text-slate-800 text-sm group-hover:text-blue-600 transition-colors">{item.name}</h3>
                                            <div className="text-xs text-slate-500 mt-1">Stok Kodu: STK-{item.id}</div>
                                        </div>
                                        <div className="text-right">
                                            <div className="font-normal text-slate-800">{(item.price * item.quantity).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} TL</div>
                            ...
                            <div className="p-4 bg-slate-50 border-t border-slate-100">
                                ...
                                <div className="flex justify-between items-center pt-3 border-t border-slate-200 mt-3">
                                    <span className="font-normal text-lg text-slate-800">Genel Toplam</span>
                                    <span className="font-normal text-xl text-blue-600">{order.amount.toLocaleString('tr-TR', { minimumFractionDigits: 2 })} TL</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                            <h3 className="font-normal text-slate-800 mb-4 flex items-center gap-2">
                                <User className="w-5 h-5 text-indigo-500" />
                                Müşteri Bilgileri
                            </h3>
                            <div className="flex items-center gap-3 mb-4 p-3 bg-indigo-50 rounded-lg border border-indigo-100">
                                <div className="w-10 h-10 bg-white text-indigo-600 rounded-full flex items-center justify-center font-normal shadow-sm">
                                    {order.customer.charAt(0)}
                                </div>
                                <div>
                                    <div className="font-normal text-indigo-900 text-sm">{order.customer}</div>
                            ...
                            <div className="mt-4 pt-3 border-t border-slate-100 flex gap-2">
                                <button className="flex-1 py-2 text-xs font-normal text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded transition-colors">Mesaj Gönder</button>
                                <Link to="/admin/users" className="flex-1 py-2 text-xs font-normal text-slate-600 bg-slate-100 hover:bg-slate-200 rounded transition-colors text-center">Profile Git</Link>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                            ...
                            <div className="flex justify-between items-center mb-2">
                                ...
                                <span className="text-sm font-normal text-slate-800">{order.paymentMethod || 'Kredi Kartı'}</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-sm text-slate-500">Durum:</span>
                                <span className="text-xs font-normal text-green-700 bg-green-100 px-2 py-1 rounded">Ödendi</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div id="printable-receipt" className="hidden">
                <div className="text-center mb-8">
                    ...
                    <h2 className="text-xl font-normal text-black uppercase tracking-widest">Sipariş Fişi</h2>
                    <p className="text-xs text-gray-500 mt-1">#{order.orderNo} - {order.date}</p>
                </div>

                <div className="border-b-2 border-dashed border-gray-300 pb-4 mb-4">
                    <div className="flex justify-between text-sm mb-1">
                        <span className="font-normal">Müşteri:</span>
                        <span>{order.customer}</span>
                    </div>
                    <div className="flex justify-between text-sm mb-1">
                        <span className="font-normal">Ödeme:</span>
                        <span>{order.paymentMethod || 'Kredi Kartı'}</span>
                    </div>
                    <div className="mt-2 text-sm">
                        <span className="font-normal block mb-1">Teslimat Adresi:</span>
